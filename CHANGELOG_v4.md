...
### バグ修正・安定性向上
- **musiccenterdir フォルダ自動作成機能**
  - 作業完了後にフォルダが削除される場合でもエラーが発生しないように改善
  - `ConfigManager.get_directory()` に フォルダ自動作成処理を追加
  - 存在しないフォルダは取得時に自動的に再作成
  - 作成失敗時は WARNING ログを出力して処理を継続
