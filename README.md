...
- **一括自動処理**: 複数アルバムを選択し、Step 4~7（AAC/Opus取り込み、アートワーク最適化・埋め込み、FLAC最終転送・作業フォルダのゴミ箱移動）をバックグラウンドで一括かつ安全に自動実行可能


### Step 2 の現行仕様

- **Demucs 自動起動**: 設定画面で Demucs 実行ファイル（StemRoller等）のパスを設定しておくことで、実行時に自動で起動します。
- **フォルダの自動オープン**: 「Demucs実行」、「Colabを開く」、および「ファイル隔離」ボタンを押した際、処理対象のFLACが配置されている `_flac_src/[アルバム名]` フォルダを自動的にエクスプローラーで開きます（Windowsのパス形式に完全対応）。
- **ファイルの自動一時退避と隔離**: 処理対象外（チェックオフ）のファイルは自動的に `demucs_ignore` フォルダに一時退避され、処理対象のみが処理フォルダに残ります。「Colabを開く」および単体の「ファイル隔離」ボタンでもこの隔離処理が自動実行されます。
- **処理済みファイルの自動探索とフィルタリング（重複排除）**: ローカルでのファイル取り込み時、まず `_flac_src` 配下に既に処理済みのファイル（`no_vocals.wav`など）が存在するか自動探索し、見つかった場合はファイル選択ダイアログを表示せず自動で取り込みを行います。また、Demucs出力フォルダに他アルバムのファイルが混在している場合や複数アルバムを一括処理した場合でも、アクティブなアルバムの曲名と一致するインストファイル（かつ曲名の重複を排除した一意なファイル）だけを自動的にフィルタリングしてインポートします。

### Step 3 の現行仕様

- `originalFile` と実ファイルの紐づけは、トラック番号・タイトル正規化・一意候補を優先して決定します。
- **Mp3tag 表示の最適化**: Mp3tag へは自動生成されたインストゥルメンタルファイルを除外した一時プレイリスト（`_mp3tag_target.m3u8`）を渡すことで、オリジナルファイルのみを表示・編集できるように制御し、タグの誤コピー等を防ぎます。
- **一時プレイリストの自動削除**: Mp3tagの終了時、エラー時、およびStep 7の最終ファイル移動時には、不要になった一時プレイリストファイル（`_mp3tag_target.m3u8`）が自動でクリーンアップされます。
- **重複しないインストトラック番号採番**: 自動生成されたインストファイルのトラック番号は、該当ディスクの元の最終トラック番号の後ろに、元のトラックの並び順（元のトラック番号順）を維持したまま連番で自動採番されます。
- **判定キーワードの最適化**: ドラマトラックやボーナストラックがインストゥルメンタルファイルと誤分類されないよう、自動判定ロジックを最適化しました。
...
- **ExternalOutputDir**: AAC/Opus変換後のファイルを保存する親フォルダ
  ...
  - **自動フォルダ切り替え機能**: Step 4/5 の取り込み時に、出力先親フォルダ（`foobar2000` や `変換MediaHuman`）を指定した場合、直下に音楽ファイルが存在せず、アルバム名のサブフォルダが存在すれば自動的にそのサブフォルダを検出して取り込みを行います。
